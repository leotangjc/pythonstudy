...
# 面向对象
	...
	# 继承和多态
		...
		# 继承的第一个好处就是子类获得了父类的全部功能（继承）
		# 第二个好处是子类和父类存在相同的方法时，会调用子类的方法（多态）
	# 获取对象信息
		# 基本类型可以使用type()判断
		# 关于class的继承类型，可以使用isistance()，如isinstance(h, Dog)，就是查询h是不是Dog类型
		# 使用dir()函数可以获取一个对象的所有属性和方法

# IO编程
	# 读文件
		# 使用open函数可打开文件，如f = open('/Users/tangjiacheng/studydata/python/test1.txt','r')。就是将指定目录下的文件，用标示符'r'来读取，并指向f
